...
class VendorOrderPickupExport implements FromCollection,WithProperties,ShouldAutoSize,WithStrictNullComparison,WithColumnWidths,WithStyles,WithTitle
{
    protected $param;
    ...
    */
    ...
    {
        $param = $this->param;
        $orders = Order::with('user','shippingMethod','items')->whereIn('id',$param['id'])->withTrashed()->get();
        if($orders){
            $x=0;
            $c=0;
            foreach ($orders as $order) {
                $oc = 0;
                $orderStart[$c] = $x + 1;
                $data[$x+0] = ['iCarry 訂單編號: '.$order->order_number,'','','','','','訂購人: '.$order->user->name];
                $data[$x+1] = ['訂單日期: '.substr($order->created_at,0,10),'','','','','','物流: '.$order->shippingMethod->name];
                $data[$x+2] = ['貨號','商家','品名','單位','數量','撿貨','確認'];
                $i = 3; //第四筆開始 items 資料
                $totalQuantity = 0;
                $itemStart[$c] = $x + $i;
                foreach($order->items as $item){
                    if($item->vendor_id == $param['vendor_id']){
    ...
    public function styles(Worksheet $sheet)
    {
        ...
        for($i=0; $i<count($orderStart); $i++){
            ...
            $sheet->getStyle('G'.($start+1))->getFont()->setSize(16);
            $sheet->getStyle('G'.($start+1))->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_RIGHT);
    ...
    public function properties(): array
    {
        return [
            ...
            'title'          => 'iCarry商家後台管理-撿貨單資料匯出',
            'description'    => 'iCarry商家後台管理-撿貨單資料匯出',
            'subject'        => 'iCarry商家後台管理-撿貨單資料匯出',
            'keywords'       => '',
            'category'       => '',
