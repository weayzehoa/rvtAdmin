    ...
    */
    public function collection()
    {
        ...
        if(isset($param['id'])){
            ...
            if(!empty($orderIds)){
                //找到所有訂單的items
                $items = OrderItemDB::with('order','vendor')->join('orders','orders.id','order_items.order_id')
                        ->join('vendors','vendors.id','order_items.vendor_id')->whereIn('orders.id',$orderIds)
                        ->select([
                        ...
                        ])->orderBy('order_items.quantity','desc')->groupBy('order_items.product_model_id')->get();
