                            ...
                            <div class="float-left">
                                <div class="input-group">
                                    <div class="input-group-append">
                                        ...
                                        <a href="{{ route('admin.admins.export') }}" class="btn btn-sm btn-success"><i class="fas fa-file-export"></i>匯出</a>
                                        @endif
                                    </div>
                                </div>
                            </div>
                            ...
                            <table class="table table-hover table-sm">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="text-center" width="10%">建立日期</th>
                                        <th class="text-center" width="10%">停用日期</th>
                                        <th class="text-center" width="10%">密碼變更日期</th>
                                        @if(in_array($menuCode.'O',explode(',',Auth::user()->power)))
                                        <th class="text-center" width="15%">啟用狀態</th>
                                        @endif
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-left align-middle">
                                            <div class="user-panel d-flex">
                                                <div class="info">
                                                    <span class="username"><a href="{{ route('admin.admins.show', $admin->id ) }}">{{ $admin->name }}</a></span>
                                                </div>
                                                @if(in_array($menuCode.'M',explode(',',Auth::user()->power)))
                                                @if($admin->lock_on == 3)
                                                <form class="d-inline" action="{{ route('admin.admins.unlock', $admin->id) }}" method="POST">
                                                    @csrf
                                                    <span>(已鎖定)</span>
                                        ...
                                        <td class="text-center align-middle">{{ explode(' ',$admin->created_at)[0] }}</td>
                                        <td class="text-center align-middle">{{ $admin->is_on == 0 ? (!empty($admin->off_time) ? explode(' ',$admin->off_time)[0] : explode(' ',$admin->updated_at)[0]) : '' }}</td>
                                        <td class="text-center align-middle">{{ explode(' ',$admin->pass_change)[0] }}</td>
                                        @if(in_array($menuCode.'O',explode(',',Auth::user()->power)))
                                        <td class="text-center align-middle">
                                            <form class="d-inline" action="{{ url('admins/active/' . $admin->id) }}" method="POST">
                                                @csrf
