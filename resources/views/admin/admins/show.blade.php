...
<div class="content-wrapper">
    ...
    <section class="content">
        <div class="container-fluid">
            @if(isset($admin))
            <form id="myform" action="{{ route('admin.admins.update', $admin->id) }}" method="POST">
            <input type="hidden" name="_method" value="PATCH">
            @else
            <form id="myform" action="{{ route('admin.admins.store') }}" method="POST">
                @endif
                @csrf
                                    ...
                                    <div class="form-group col-md-3">
                                        ...
                                        <div class="form-group clearfix">
                                            <div class="icheck-danger d-inline mr-2">
                                                <input type="checkbox" id="active_lock" name="lock_on" value="3" {{ isset($admin) ? $admin->lock_on >= 3 ? 'checked' : '' : '' }} {{ old('lock_on') >= 3 ? 'checked' : '' }}>
                                                <label for="active_lock">鎖定</label>
                                            </div>
                                            <div class="icheck-green d-inline mr-2">
                                                <input type="radio" id="active_pass" name="is_on" value="1" {{ isset($admin) && $admin->is_on == 1 ? 'checked' : '' }}>
...
@section('JsValidator')
{!! JsValidator::formRequest('App\Http\Requests\Admin\AdminsUpdateRequest', '#myform'); !!}
@endsection
    ...
    function toggleSelect(parentId, trigger) {
        ...
        if (trigger.checked) {
            option_list.forEach((el) => {
                if (!el.classList.contains("disabled") && el.getAttribute("type") == "checkbox" && el.getAttribute("name") != "lock_on") {
                    el.checked = true;
                }
            });
        } else {
