            ...
            <div class="row">
                <div class="col-12">
                    <div class="card">
                                ...
                                <form action="{{ url('exportcenter') }}" method="GET" class="form-inline" role="search">
                                    <div class="form-group row">
                                        <div class="form-group-sm">
                                            <span class="mr-2">匯出者：{{ auth('admin')->user()->name }}</span>
                                        </div>
                                        <div class="form-group-sm">
                        ...
                        <div class="card-body">
                            <div class="mb-2"><span class="text-danger text-bold">注意！匯出中心僅能看到匯出者自己的資料，資料僅保留兩周，若未手動刪除，系統將自動清除超過兩周前的資料。</span></div>
                            ...
                            <table class="table table-hover table-sm">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                            ...
                                            @elseif(!empty($export->condition['con']))
                                                ...
                                                @elseif($export->condition['model'] == 'orders')
                                                    ...
                                                    @else
                                                        ...
                                                        @endif
                                                    @endif
                                                    @if(!empty($export->cons['pay_method']) && $export->cons['pay_method'] != '全部')<span class="badge badge-info mr-1">付款方式：{{ $export->cons['pay_method'] == '全部' ? '全部' : str_replace('全部,','',$export->cons['pay_method']) }}</span>@endif
                                                    ...
                                                    @if(!empty($export->cons['vendor_name']))<span class="badge badge-info mr-1">收件人地址：{{ $export->cons['vendor_name'] }}</span>@endif
                                                    @if(!empty($export->cons['product_name']))<span class="badge badge-info mr-1">商品名稱：{{ $export->cons['product_name'] }}</span>@endif
                                                @endif
                                            @endif
                                        </td>
                                        <td class="text-center align-middle">
                                            <a href="{{ asset('exports/'.$export->filename) }}" class="btn btn-sm {{ strstr($export->filename,'pdf') ? 'btn-danger' : 'btn-success' }}" title="{{ $export->filename }}" {{ strstr($export->filename,'pdf') ? 'target="_blank"' : '' }}>
                                        ...
                                        <td class="text-center align-middle">
                                            <form action="{{ route('admin.exportcenter.destroy', $export->id) }}" method="POST">
                                                @csrf
                                                <input type="hidden" name="_method" value="DELETE">
